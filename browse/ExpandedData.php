...
<div class="container">
    ...
    <div class="row">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Expanded_AMPs</li>
...
$query_mysql_one="SELECT DRAMP_ID,Name,Sequence,Activity FROM expanded_amps";

$query_mysql_two="SELECT count(DRAMP_ID) FROM expanded_amps";
...
echo<<<END_END
	...
	<tr><th style="border-right-style: none"><input id="CheckAll" onclick="SelectAll('browse_parent','browse_child')" name="browse_parent" type="checkbox" /></th><th style="border-left-style:none;width:90px;">DRAMP ID</th><th>Peptide NAME</th><th>Peptide Sequence</th><th>Biological Activity</th></tr>
...
for ($i=0;$i<count($fengyepage->res_array);$i++){
	...
	if ($i%2 == 0) {
			...
			echo "<tr class='altrow'><td style='border-right-style: none'><input type='checkBox' name='browse_child' value='{$row['DRAMP_ID']}' onclick='my_blast()'/></td><td style='border-left-style: none'><a href='http://dramp.cpu-bioinfor.org/browse/Expanded_information.php?id={$row['DRAMP_ID']}&dataset=Expanded'>{$row['DRAMP_ID']}</a></td><td>{$row['Name']}</td><td style='font-family:Segoe UI Symbol'>{$row['Sequence']}</td><td>{$row['Activity']}</td></tr>";
