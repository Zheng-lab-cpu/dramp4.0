...
$mkdir_query = "mkdir -p -m 755 /var/www/dramp/tmp/predict_tmp/$file_name";
...
#the template
my $content_line;
	open LOL,"</var/www/dramp/template/tools_result_static_predict.html";
        	while (my $line = <LOL>){
                	$content_line .= $line;
        	}
	close LOL;
...
#			http://www.biogem.org/cgi-bin/cho-fas.pl?sequence=%3ESSSDDDFF%0D%0ATTPATTSSWTCITAGVTVSASLCPTTKCTSRC&submit=PREDICT
		...
		open (LOL,">/var/www/dramp/tmp/predict_tmp/$file_name/$file_name.htm");
...
sub physic_predict{
	...
	#			$physic_predict =~ s/\<\/pre\>\<\!\-\-.*/\<\/pre\>/;
				...
				open (DOTA,">/var/www/dramp/tmp/predict_tmp/$file_name/$file_name.htm");
